...
function Window:CreateWindow( titletext, size, tabcolor, windowcolor, titletextcolor, contenttext )
    ...
    window_tab:Subscribe( "Render", self, function() if window_tab:GetHovered() and Key:IsDown(1) then window:SetPosition( Vector2( Mouse:GetPosition().x - window:GetSize().x / 2, Mouse:GetPosition().y - window_tab:GetSize().y / 2 ) ) end end )
